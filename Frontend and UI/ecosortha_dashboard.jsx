...
function calcThermalRisk(adjustedTemp) {
  if (adjustedTemp > 35)
    return { value: 1.0, label: "Critical", color: ACCENT.red };
  if (adjustedTemp > 32)
    return { value: 0.5, label: "Moderate", color: ACCENT.amber };
...
function calcTST(trustScore, zone, packaging, hour) {
  const uhi = UHI_ZONES[zone];
  const pkgFactor =
    packaging === "thermal" ? 4.0 : packaging === "insulated" ? 2.0 : 1.0;
  const solarMulti = getSolarHourMultiplier(hour);
  const raw =
    ((trustScore * pkgFactor * uhi.baseSurvival) /
      (uhi.hazardMultiplier * solarMulti)) *
    60;
...
function ScoreGauge({ value, label, size = 140 }) {
  const color =
    value >= 75 ? ACCENT.green : value >= 55 ? ACCENT.amber : ACCENT.red;
  return (
    <div
      style={{
        display: "flex",
    ...
    <div>
      ...
      >
        ...
        <div>
          <SliderRow
            ...
            onChange={setTemp}
            unit="°C"
            color={temp > 35 ? ACCENT.red : ACCENT.amber}
          />
          <div style={{ fontSize: 10, color: "var(--text-dim)" }}>
            Optimal: 25–32°C
          </div>
      ...
      >
        {isLoading
          ? "⏳ Calculating..."
          : ts >= 60
            ? "✦ Certify Batch & Generate QR Certificate"
            : "Trust Score too low to certify"}
      </button>
      {certified && (
        <div
          style={{
        ...
        >
          ...
          >
            ✓ BATCH CERTIFIED — Certificate Generated
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
            ...
            >
              <div>
                <span style={{ color: ACCENT.green }}>Batch #:</span> BCH-
                {Date.now().toString().slice(-6)}
              </div>
              <div>
                <span style={{ color: ACCENT.green }}>Trust Score:</span> {ts}
                /100
              </div>
              <div>
                <span style={{ color: ACCENT.green }}>pH / EC / Temp:</span>{" "}
                {pH} / {EC} / {temp}°C
              </div>
              <div>
                <span style={{ color: ACCENT.green }}>Ratio / Days:</span>{" "}
                {ratio} / {days}
              </div>
              <div>
                <span style={{ color: ACCENT.green }}>Processor:</span> #07
                (anonymized)
              </div>
            </div>
          </div>
...
function DispatchCalendar({ baseTemp, zone, trustScore, windSpeed }) {
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      >
        24-Hour Dispatch Safety Window
      </div>
      <div
        style={{
          display: "grid",
      ...
      >
        {hours.map(({ h, dvs }) => {
          const col =
            dvs >= 75 ? ACCENT.green : dvs >= 55 ? ACCENT.amber : ACCENT.red;
          return (
            <div
              key={h}
              title={`${h}:00 — DVS ${dvs}`}
      ...
      <div style={{ display: "flex", gap: 14, marginTop: 10, fontSize: 11 }}>
        {[
          [ACCENT.green, "Safe (DVS≥75)"],
          [ACCENT.amber, "Caution (55–74)"],
          [ACCENT.red, "High Risk (<55)"],
        ].map(([c, l]) => (
          <span
            key={l}
            style={{
...
function MicroclimateSimulator({ trustScore }) {
  ...
  const filteredZones = Object.keys(UHI_ZONES).filter((z) =>
  ...
  const startSpeechRecognition = (lang) => {
    ...
    recognition.onresult = (event) => {
      ...
      const matchedZone = Object.keys(UHI_ZONES).find((z) =>
        z.toLowerCase().includes(transcript.toLowerCase()),
      );
      if (matchedZone) {
        setZone(matchedZone);
      }
        ...
        <div>
          ...
          >
            <div style={{ position: "relative", flex: 1 }}>
              <span
                style={{ position: "absolute", left: 12, top: 9, fontSize: 13 }}
              >
                🔍
              </span>
              <input
              ...
              return (
                ...
                >
                  ...
                  <div style={{ fontSize: 9, opacity: 0.8, marginTop: 2 }}>
                    +{d.offset}°C · Class {d.hazardClass}
                  </div>
                </button>
              );
            })}
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  background:
                    packaging === v ? ACCENT.greenBg : "var(--bg-input)",
          ...
          >
            ...
            >
              <div style={{ color: "var(--text-secondary)" }}>
                TST = (Trust × Insulation)
              </div>
              <div style={{ color: "var(--text-secondary)", marginLeft: 24 }}>
                ÷ (Hazard × Solar)
              </div>
              <div
                style={{
...
function DemandChart() {
  ...
  useEffect(() => {
    ...
    const gridColor =
      rootStyles.getPropertyValue("--gauge-track").trim() || "#1E293B";
    const tickColor =
      rootStyles.getPropertyValue("--text-dim").trim() || "#64748B";
  ...
  return (
    <div>
      ...
      <div style={{ position: "relative", width: "100%", height: 200 }}>
        ...
        >
          30-day demand forecast with climate event markers
        </canvas>
      </div>
      <div
        style={{
      ...
      >
        ...
        <div>
          <div style={{ fontSize: 12, color: ACCENT.amber, fontWeight: 700 }}>
            Climate Demand Alert
          </div>
          <div
            style={{
              fontSize: 11,
          ...
          >
            Heatwave forecast in 5 days → expect +40% demand for cold-chain
            sensitive products. Prepare additional inventory and upgrade
    ...
    >
      {[
        [
          "🛡️",
          "Spoilage Prevented",
          `BDT ${spoilagePrevented.toLocaleString()}`,
          ACCENT.green,
        ],
  ...
  const hazardMaps = [
    {
      ...
      temp: "41.2°C",
      rh: "74%",
      desc: "Class A thermal accumulation zone. Narrow concrete corridors trap heat.",
    ...
    },
    {
      ...
      temp: "38.7°C",
      rh: "68%",
      desc: "Dense residential concrete with limited canopy cover.",
    ...
    },
    {
      ...
      temp: "36.1°C",
      rh: "62%",
      desc: "Mixed urban with partial green canopy. Moderate risk window.",
    ...
    },
    {
      ...
      temp: "34.2°C",
      rh: "58%",
      desc: "High green canopy coverage and lake proximity reduce thermal load.",
      time: "N/A",
    },
  ];
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24 }}>
        <Card hover={false}>
          ...
          >
            ...
            >
              Dhaka Thermal Hazard Map
            </div>
            <div style={{ fontSize: 12, color: "var(--text-secondary)" }}>
              Live MERM Classification
            </div>
          </div>
            ...
            {hazardMaps.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  {m.time !== "N/A" && (
                    <div style={{ fontSize: 12, color: ACCENT.amber }}>
                      Peak: {m.time}
                    </div>
                  )}
                </div>
        ...
        <Card hover={false}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {[
              {
                icon: "🛡️",
                text: "Batch ECO-2025-142 certified — Trust Score 91",
                time: "2 min ago",
                bg: ACCENT.greenBg,
              },
              {
                icon: "📈",
                text: "DVS simulation for Old Dhaka route — Score 63 (Caution)",
                time: "18 min ago",
                bg: ACCENT.amberBg,
              },
              {
                icon: "🚚",
                text: "Batch ECO-2025-140 dispatched to Mirpur",
                time: "1 hr ago",
                bg: ACCENT.greenBg,
              },
              {
                icon: "⚠️",
                text: "High thermal hazard in Old Dhaka — delay dispatches until 5 PM",
                time: "2 hr ago",
                bg: ACCENT.redBg,
              },
              {
                icon: "📦",
                text: "New biochar batch ECO-2025-141 created (180 kg)",
                time: "3 hr ago",
                bg: ACCENT.blueBg,
              },
            ].map((a, i) => (
  ...
  return (
    <div style={{ animation: "fadeSlideIn 0.3s ease" }}>
      <PageHeader
        ...
        subtitle="All bio-resource batches with certification status"
        action={
          <button
            onClick={onNewBatch}
            style={{
            ...
            {batches.map((b, i) => (
              ...
              >
                ...
                <td style={{ padding: "18px 24px" }}>
                  <span
                    style={{
                      border: `1px solid ${b.status === "Certified" ? ACCENT.greenBorder : "var(--border-primary)"}`,
                      color:
                        b.status === "Certified"
                          ? ACCENT.green
                          : "var(--text-secondary)",
      ...
      <Card style={{ maxWidth: 700 }} hover={false}>
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
          ...
          >
            ...
            >
              After creating your batch, submit IoT sensor readings to generate
              a BARI-compliant Trust Score and cryptographic QR certificate.
            </div>
          </div>
  ...
  const [messages, setMessages] = useState([
    {
      role: "system",
      content:
        "Hello! I am EcoSortha AI, your voice and text-based assistant. I can provide microclimate forecasts, smart dispatch suggestions, or analyze files and context. How can I help you today?",
  ...
  return (
    ...
    >
      ...
      >
            ...
            >
              <div
                style={{
                  ...
                  padding: "12px 18px",
                  borderRadius:
                    m.role === "user" ? "18px 18px 0 18px" : "18px 18px 18px 0",
                  background:
                    m.role === "user" ? ACCENT.greenDark : "var(--bg-input)",
        ...
        >
          {["Climate Forecast", "Dispatch Suggestions", "Submit Feedback"].map(
            (action) => (
              <button
                key={action}
                onClick={() => handleSend(`Show me ${action.toLowerCase()}`)}
                style={{
                  padding: "6px 14px",
                  borderRadius: 20,
                  background: "var(--bg-primary)",
...
function MarketplaceView() {
  ...
  return (
    <div style={{ animation: "fadeSlideIn 0.3s ease", position: "relative" }}>
      <PageHeader
        title="Climate-Resilient Marketplace"
        subtitle="Source verified heat-sensitive products optimized for DVS delivery"
      />
      ...
      <Card style={{ padding: "16px 24px", marginBottom: 24 }} hover={false}>
        ...
        >
          <div style={{ flex: "1 1 250px", position: "relative" }}>
            <span
              style={{ position: "absolute", left: 14, top: 12, fontSize: 16 }}
            >
              🔍
            </span>
            <input
            ...
            {p.badge && (
              <div
                style={{
                  ...
                  top: 16,
                  right: 16,
                  background: p.badge.includes("Critical")
                    ? ACCENT.redBg
                    : ACCENT.blueBg,
                  color: p.badge.includes("Critical")
                    ? ACCENT.red
                    : ACCENT.blue,
                  padding: "4px 10px",
                  borderRadius: 4,
  ...
  return (
    <div
      style={{
        ...
        padding: 0,
        transition:
          "background 0.5s cubic-bezier(0.4,0,0.2,1), color 0.4s ease",
      }}
    >
      <link
          ...
          >
            ...
            <div>
              ...
              <Card>
                ...
                >
                  Adjust IoT parameters on the left to simulate a batch. A score
                  of 60+ is required for BARI certification and cryptographic
                  signing.
                </div>
              </Card>
            </div>
      ...
      >
        ...
        >
          ECOSORTHA AI CLIMATESHIELD · INFINITY AI BUILDFEST 2026 · TEAM GLIDERS
          · TRACK 4: E-COMMERCE
        </div>
        <div
          style={{
