...
var { useEffect, useRef } = window.React || React;
  ...
  useEffect(() => {
    ...
    const createTree = (x, z, scale = 1) => {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        const coneMat = new THREE.MeshStandardMaterial({ color: canopyColors[i], flatShading: true });
    ...
    // 5. Trees (2-3)
    const trees = [
      createTree(-2.5, -2, 1.1),
      createTree(3, -1.5, 0.95),
      createTree(-1.8, 1, 0.85)
    ];

    // Helper: Create low-poly vegetables
    const createVegetable = (type, colorVal) => {
      const vegGroup = new THREE.Group();
      const vegMat = new THREE.MeshStandardMaterial({ color: colorVal, flatShading: true });
      ...
      if (type === 'carrot') {
        ...
        const leafMat = new THREE.MeshStandardMaterial({ color: 0x059669, flatShading: true });
        ...
        const stemMat = new THREE.MeshStandardMaterial({ color: 0x047857, flatShading: true });
        ...
        const huskMat = new THREE.MeshStandardMaterial({ color: 0x84cc16, flatShading: true });
    ...
    // 6. Vegetables
    const veggies = [
      { mesh: createVegetable('carrot', 0xeab308), baseX: -1.2, baseZ: 2, offset: 0 },
      { mesh: createVegetable('pepper', 0x10B981), baseX: 1.5, baseZ: 3, offset: Math.PI / 3 },
      { mesh: createVegetable('corn', 0xf59e0b), baseX: 0.8, baseZ: 0.5, offset: Math.PI / 1.5 }
    ];
    ...
    const snoutMat = new THREE.MeshStandardMaterial({ color: 0x374151, flatShading: true });
    ...
    const legMat = new THREE.MeshStandardMaterial({ color: 0xd1d5db, flatShading: true });
    for (let i = 0; i < 4; i++) {
