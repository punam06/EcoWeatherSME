...
// Writes text to clipboard with fallback for HTTP (non-secure context).
...
// Small inline copy button — shows a brief "copied" state.
function CopyBtn({
  text,
  label
}) {
  ...
  return React.createElement('button', {
    ...
    style: {
      ...
      borderRadius: 6,
      border: '1px solid var(--border-primary)',
      background: copied ? 'rgba(16,185,129,0.15)' : 'var(--bg-input)',
...
function MicroclimateSimulator({
  ...
  // Live recalculate adjusted temp and thermal risk whenever input parameters change
  useEffect(() => {
    const live = calcAdjustedTemp(baseTemp, zone, hour, windSpeed);
    setAdjustedTemp(live);
    setThermalRisk(calcThermalRisk(live));
  }, [baseTemp, zone, hour, windSpeed]);
  ...
  }, "Analyzing microclimate...")) : !hasCalculated ? /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      alignItems: "center",
  ...
  }, "Adjusted Temp: ", /*#__PURE__*/React.createElement("strong", {
    style: {
      color: "var(--text-primary)"
    }
  ...
  }, "Adjust parameters above, then press ", /*#__PURE__*/React.createElement("strong", null, "Calculate Viability Score"), " for DVS, TST & Trust Score.")) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 24,
      justifyContent: "center",
  ...
  }, adjustedTemp.toFixed(1), "\xB0C"))))), hasCalculated && /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "20px 24px",
      background: "var(--bg-input)",
      borderRadius: 12,
  ...
  }, "\u2248 ", tst, " min"))))), hasCalculated && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 14,
      padding: 14,
      borderRadius: 10,
  ...
  }, advice)), safeComponent("DispatchCalendar", {
    baseTemp,
    zone,
    trustScore,
    windSpeed,
...
/* ═══════════════════════════════════════════════════════════════
   SME INVENTORY TRACKER — QR Scan + Climate Advisory
   ═══════════════════════════════════════════════════════════════ */

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
const BATCH_ID_RE = /^BCH-\d{6,}$/i;
function translateSaleWindowToBangla(enText, daysViable, riskLevel) {
  const hours = Math.max(6, Math.round(daysViable * 24));
  const days = Math.max(1, Math.ceil(daysViable));
  if (daysViable <= 2 || riskLevel === "EXTREME") {
    return `আগামী ${hours} ঘণ্টার মধ্যে বিক্রি সম্পন্ন করুন — অতিরিক্ত তাপমাত্রার ঝুঁকি রয়েছে। সক্রিয় জৈব যৌগ দ্রুত হ্রাস পেতে পারে।`;
  }
  if (daysViable <= 7 || riskLevel === "HIGH") {
    return `আগামী ${days} দিনের মধ্যে বিক্রি করুন — আপনার এলাকায় উচ্চ তাপমাত্রার চাপ রয়েছে।`;
  }
  if (daysViable <= 10 || riskLevel === "MEDIUM") {
    return `আগামী ${days} দিনের মধ্যে বিক্রি করার পরামর্শ দেওয়া হচ্ছে — মাঝারি জলবায়ু ঝুঁকি বিবেচনায়।`;
  }
  return `ছায়াযুক্ত অবস্থায় ${Math.floor(daysViable)} দিন পর্যন্ত নিরাপদ সংরক্ষণ সম্ভব। তবুও তাপমাত্রা পর্যবেক্ষণ করুন।`;
}
function compoundLifespanPct(daysViable) {
  return Math.max(4, Math.min(100, Math.round(daysViable / 14 * 100)));
}
function riskMeterColor(riskLevel, pct) {
  if (riskLevel === "EXTREME" || pct < 25) return ACCENT.red;
  if (riskLevel === "HIGH" || pct < 45) return "#F97316";
  if (riskLevel === "MEDIUM" || pct < 70) return ACCENT.amber;
  return ACCENT.green;
}
function ClimateAdvisoryCard({
  recommendations,
  banglaText,
  onSpeak,
  isSpeaking
  ...
  const pct = compoundLifespanPct(recommendations.days_viable);
  const meterColor = riskMeterColor(recommendations.risk_level, pct);
  return /*#__PURE__*/React.createElement(Card, {
    style: {
      marginTop: 20,
      border: `1px solid ${meterColor}33`,
      boxShadow: `0 0 24px ${meterColor}18`
    }
  }, /*#__PURE__*/React.createElement(SectionLabel, {
    icon: "\uD83C\uDF21\uFE0F",
    text: "Climate Advisory Card"
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
  ...
  }, "ACTIVE COMPOUND LIFESPAN"), /*#__PURE__*/React.createElement("div", {
    style: {
      position: "relative",
      height: 18,
      borderRadius: 999,
    ...
    style: {
      position: "absolute",
      inset: 0,
      background: `linear-gradient(90deg, ${ACCENT.red} 0%, ${ACCENT.amber} 45%, ${ACCENT.green} 100%)`,
      opacity: 0.85
    }
  }), /*#__PURE__*/React.createElement("div", {
    style: {
  ...
  }, "estimated viability")), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 8,
      flexWrap: "wrap"
  ...
  }, recommendations.risk_level, " RISK"), /*#__PURE__*/React.createElement("span", {
    style: {
      fontSize: 10,
      padding: "4px 10px",
      borderRadius: 6,
  ...
  }, recommendations.days_viable, " days viable"))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 12,
      color: "var(--text-secondary)",
      marginBottom: 8,
  ...
  }, recommendations.best_sale_window), /*#__PURE__*/React.createElement("label", {
    style: {
      display: "block",
      fontSize: 11,
      fontWeight: 700,
  ...
  }, "\u0986\u09AA\u09A8\u09BE\u09B0 \u099C\u09A8\u09CD\u09AF \u09AA\u09B0\u09BE\u09AE\u09B0\u09CD\u09B6 (AI Recommendation)"), /*#__PURE__*/React.createElement("div", {
    style: {
      padding: "14px 16px",
      borderRadius: 10,
      minHeight: 88,
      background: "linear-gradient(135deg, rgba(6,95,70,0.12), rgba(16,185,129,0.06))",
      border: `1px solid ${ACCENT.greenBorder}`,
      fontSize: 15,
      lineHeight: 1.65,
      color: "var(--text-primary)",
  ...
  }, banglaText), /*#__PURE__*/React.createElement("button", {
    onClick: onSpeak,
    title: "Read recommendation aloud (Bangla)",
    style: {
      marginTop: 12,
      display: "inline-flex",
      alignItems: "center",
      ...
      borderRadius: 8,
      cursor: "pointer",
      background: isSpeaking ? ACCENT.amberBg : ACCENT.greenBg,
      border: `1px solid ${isSpeaking ? ACCENT.amberBorder : ACCENT.greenBorder}`,
      color: isSpeaking ? ACCENT.amber : ACCENT.green,
      fontWeight: 600,
      fontSize: 13
    }
  }, /*#__PURE__*/React.createElement("span", {
  ...
  }, isSpeaking ? "🔊" : "🔈"), isSpeaking ? "পড়া হচ্ছে…" : "শুনুন (Text-to-Speech)"))));
}
function SMEInventoryTracker({
  activeZone = "Mirpur"
}) {
  ...
  useEffect(() => {
    ...
    const fromGlobal = typeof window !== "undefined" ? window.__QR_PRODUCT__ : null;
    const candidate = fromUrl || fromGlobal;
    if (candidate && (UUID_RE.test(candidate) || BATCH_ID_RE.test(candidate))) setBatchUuid(candidate);
  }, []);
  const banglaText = claimResult?.recommendations ? translateSaleWindowToBangla(claimResult.recommendations.best_sale_window, claimResult.recommendations.days_viable, claimResult.recommendations.risk_level) : "";
  const resolveGps = () => new Promise(resolve => {
  ...
  const executeClaim = async uuid => {
    ...
    if (!UUID_RE.test(trimmed) && !BATCH_ID_RE.test(trimmed)) {
      setError("Enter a valid Batch UUID or BCH- batch number (e.g. BCH-202659).");
      return;
    }
    setError(null);
    setIsClaiming(true);
    ...
    try {
      ...
      const json = await window.APIClient.claimSMEInventory(trimmed, {
        zone: activeZone,
        storage_condition: "shaded",
        ...gps
      });
      if (!json.success || !json.recommendations) {
        throw new Error(json.error || "SME claim failed");
      }
      setClaimResult(json);
      if (typeof window.showToast === "function") {
        window.showToast("✅ Batch saved to SME inventory successfully!", "success");
      }
    } catch (err) {
      const msg = err?.message || "Failed to claim batch";
      setError(msg);
      if (typeof window.showToast === "function") {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    title: "SME Inventory Intake",
    subtitle: "Scan a producer QR code to claim custody and receive AI climate sale-window guidance"
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "grid",
    ...
    style: {
      ...
      aspectRatio: "4/3",
      background: "#0a0f1a",
      border: `2px solid ${isScanning ? ACCENT.green : "var(--border-primary)"}`,
  ...
  }, "BATCH UUID (SIMULATED SCAN PAYLOAD)"), /*#__PURE__*/React.createElement("input", {
    type: "text",
    value: batchUuid,
    ...
    style: {
      ...
      fontWeight: 600,
      fontSize: 13,
      opacity: !batchUuid.trim() || isClaiming ? 0.5 : 1
    }
  }, "Claim Directly")), error && /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 12,
      padding: "10px 14px",
      borderRadius: 8,
    ...
    value: "Shaded warehouse",
    icon: "🏪"
  }, {
    label: "API Endpoint",
    value: "POST /api/qr/sme-claim/:id",
    icon: "🔗"
  ...
  }, row.label.toUpperCase()), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 13,
      color: "var(--text-primary)",
      fontWeight: 500,
  ...
  }, "Scanning triggers inventory save (", /*#__PURE__*/React.createElement("code", {
    style: {
      color: ACCENT.green
    }
  }, "sme_inventory"), "), logs GPS/IP to the custody ledger, and runs the predictive sale-window engine using live weather + UHI offsets."))), claimResult?.product_saved && /*#__PURE__*/React.createElement(ClimateAdvisoryCard, {
    recommendations: claimResult.recommendations,
...
// Safe component renderer — prevents ReferenceError crashes when a component
// definition is missing (e.g. not yet ported from climalogix_dashboard.jsx).
// Uses window[name] string lookup instead of direct reference to avoid
// ReferenceError on undeclared identifiers.
function safeComponent(name, props, displayName) {
  var Component;
  try {
  ...
  }, '\u{1F6A7}'), React.createElement('div', {
    style: {
      fontSize: 13,
      color: 'var(--text-dim)'
    }
  }, '\u201C' + label + '\u201D component is not loaded yet.'), React.createElement('div', {
    style: {
      fontSize: 11,
      color: 'var(--text-muted)',
      marginTop: 8
...
function CLimaLogixApp() {
  ...
  if (selectedSme === "custom_sme" && (userRole === "buyer" || userRole === "sme" || userRole === "sme_owner")) {
    activeTabs.push({
      id: "configurator",
  ...
  if (!currentUser) {
    ...
    }), showAuthOverlay && /*#__PURE__*/React.createElement(AuthPanelComponent, {
