...
app.get('/api/test-groq', asyncHandler(async (req, res) => {
  ...
  try {
    ...
    report.rawFetchStatus = rawRes.status;
    report.rawFetchOk = rawRes.ok;
    
    const rawBody = await rawRes.text();
    report.rawFetchBody = rawBody;
  } catch (err) {
    report.rawFetchError = {
      message: err.message,
      stack: err.stack,
      code: err.code
    };
  ...
  try {
    ...
    report.groqSdkOk = true;
    report.groqSdkResult = completion.choices[0]?.message?.content;
  } catch (err) {
    report.groqSdkOk = false;
    report.groqSdkLibraryError = {
      message: err.message,
      stack: err.stack,
      code: err.code,
      status: err.status
  ...
  res.json({ success: report.rawFetchOk && report.groqSdkOk, report });
}));
